var html5Forms = new function () {
	...
	var scriptNode = null,
		...
		hasNativeBubbles = userAgent.indexOf('WebKit') < 0 || parseInt(userAgent.match(/AppleWebKit\/([^ ]*)/)[1].split('.')[0])  > 534,
	...
	me.init = function () {
			...
			if (window.yepnope) {
				...
				if (toLoad.length == 0) {
					...
					if (document.addEventListener) {
						document.addEventListener('DOMContentLoaded', me.setupExtraFeatures, false);
					}
				} else {
					yepnope({
						load: toLoad,
		 ...
		 */
		me.setupExtraFeatures = function () {
