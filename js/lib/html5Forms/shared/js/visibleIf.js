...
var visibleIf = new function(){
    ...
    me.init = function(reset){
        ...
        requiredIfNodes = CSSHelpers.getElementsByClassName(document, 'requiredIf');
    ...
    me.refreshPage = function(options){
        ...
        if (!(options && !options.isPageLoaded)) {
            ...
            for (var i = 0; i < requiredIfNodes.length; i++) {
                setRequiredState(requiredIfNodes[i], options);
    ...
    function setRequiredStates(e){
        changedInput = this;
        for (var i = 0; i < requiredIfNodes.length; i++) {
            setRequiredState(requiredIfNodes[i]);
    ...
    function setEvents(){
        ...
        visibleIfNodes = CSSHelpers.getElementsByClassName(document, 'visibleIf');
        requiredIfNodes = CSSHelpers.getElementsByClassName(document, 'requiredIf');
        var nodesToIndex = [visibleIfNodes, requiredIfNodes];
        var nameCounter = 0;
    ...
    function isVisible(node){
    	return (node.offsetWidth != 0 || node.type === 'hidden');
    }
    ...
    function setRequiredState(node){
    	var rule = getRule(node, 'requiredIf-rule')
        ...
        if (isRuleTrue) {
        	CSSHelpers.addClass(node, 'requiredIf-required');
            DOMHelpers.setAttributeValue(node, 'required', 'required');
            //setFormElementsInside(node, false);
        } else {
        	CSSHelpers.removeClass(node, 'requiredIf-required');
