...
$wf2 = {
	...
	init : function(reinit){
		...
		if (!reinit) {
			...
			if(document.addEventListener){
				...
				window.addEventListener('resize', $wf2.clearInvalidIndicators, false);
			}
				...
				document.attachEvent('resize', $wf2.clearInvalidIndicators);
			}
		}
	...
	hiliteFirstError: function () {
		...
		if($wf2.invalidIndicators.length){ //second condition needed because modal in oninvalid handler may cause indicators to disappear before this is reached
			var errorMsgNode = $wf2.invalidIndicators[0].errorMsg;
			
			//$wf2.invalidIndicators[0].errorMsg.className += " wf2_firstErrorMsg";
			if (!$wf2.isIOS) {
				$wf2.css.addClass(errorMsgNode, "wf2_firstErrorMsg");
			}
			...
			if ($wf2.isIOS) {
				// there are a whole bunch of things to consider if showing a bubble
				// so we will not do this and use alerts instead.
				alert(errorMsgNode.innerText);
...
EventHelpers.addPageLoadEvent('$wf2.init', true)
